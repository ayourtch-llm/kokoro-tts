...
- State-dict load: `alpha1.{0,1,2}` and `alpha2.{0,1,2}` are stored *directly* as parameters (no `.weight` suffix — they're `nn.Parameter`, not `nn.Linear`). **Candle VarBuilder gotcha** (codex hit this empirically at stage 12 / Snake1D, a313a52): the correct call is `vb.get((1, channels, 1), "alpha1.0")?` — pass the full key as the *name* argument to `get`. **Do NOT** use `vb.pp("alpha1.0").get((1, channels, 1), "")?` — pushing into the prefix and asking for an empty name does not resolve a bare `nn.Parameter` key in candle 0.8. This applies to *any* bare-Parameter port (no Linear/Conv wrapper); it's not Snake1D-specific. The `.pp()` + empty-name pattern works for some Module-wrapper conventions but not for raw parameters under a flat key.
