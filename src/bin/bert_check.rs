//! Validate stage 3 CustomAlbert against `tools/reference_custom_albert.py`.

use anyhow::{bail, Context, Result};
use candle_core::{DType, Device, Tensor};
use candle_nn::VarBuilder;
use kokoro_tts::model::bert::CustomAlbert;
    ...
    fn parse() -> Result<Self> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    println!(
                        "usage: cargo run --bin bert_check -- --model PATH --config PATH --ref PATH [--phonemes IPA] [--atol F32]"
                    );
                    std::process::exit(0);
                }
...
fn main() -> Result<()> {
    ...
    let attention_mask = Tensor::ones((1, ids.len()), DType::U8, &device)?;

    let vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&[args.model.clone()], DType::F32, &device)
            .context("loading model safetensors")?
    };
    let model = CustomAlbert::load(vb.pp("bert"), config.to_albert_config())
